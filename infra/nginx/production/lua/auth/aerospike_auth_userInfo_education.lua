...
function returnToClientEducation ( status, errorcode, message, title, oauthres )
	local resBody =  {
                error  =  message,
                status =  {
                        result =  "failure",
...
local ssoToken, formSet = nil, "education_token"

if( ngx.var.arg_sso_token == nil ) then ssoToken = ngx.req.get_headers().sso_token else ssoToken = ngx.var.arg_sso_token end 

if( ssoToken == nil )
then
	returnToClientEducation(401, "GE_OA_4000", "Auth token missing", "Login Required")
end
...
then
	...
	if res.status ~= 200 or truncated or res.body == nil then
		returnToClientEducation(410, "GE_OA_4002", "Failed to validate user details. Please try again.", "Error", tonumber(responseCode))
	end

	if( value == nil or value.status == 401 or value.status == 410 ) then
		returnToClientEducation(401, "GE_OA_4001", "Looks like you have been signed out. Please login again and retry.", "Login Required", tonumber(responseCode))
	end
	...
	local userInfo, userEmail, userFirstname, userLastName, userPhone = value.defaultInfo, "", "", "", ""

	if( userInfo ~= nil) then
		userEmail = userInfo.email~= nil and userInfo.email or ""
		userFirstname = userInfo.firstName~= nil and  userInfo.firstName or ""
		userLastName = userInfo.lastName~= nil and  userInfo.lastName or ""
	...
	ngx.req.set_header("X-USER-LASTNAME",userLastName)
    ngx.req.set_header("X-USER-PHONE",userPhone)	

--	aerospike is not saving mobile number correctly due to integer overflow, hence not storing currently	
--	as.put(cluster, namespace, formSet, ssoToken, 4, {userId = value.userId, email = userEmail, firstName = userFirstname, lastName = userLastName, phone = userPhone})
    ...
    ngx.req.set_header("X-USER-LASTNAME",r.lastName)
