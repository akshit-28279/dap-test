...
local function push_data(premature, message)
    local conf = { host = "127.0.0.1", port = 8125, namespace = "Lua_Stats_App_Education_Explore", timeout = 1}
    ...
    if(starts_with(message.uri, "/explore/v1/page/")) then
        maskUri = "/explore/v1/page"
    elseif(starts_with(message.uri, "/explore/auth/v1/institute/")) then
        maskUri = "/explore/auth/v1/institute"
    elseif (starts_with(message.uri, "/explore/auth/v1/exam/")) then
        maskUri = "/explore/auth/v1/exam"
    elseif (starts_with(message.uri, "/explore/auth/v1/course/")) then
        maskUri = "/explore/auth/v1/course"
    elseif(starts_with(message.uri, "/v1/institute/exam_list/")) then
        maskUri = "/v1/institute/exam_list"
    end
